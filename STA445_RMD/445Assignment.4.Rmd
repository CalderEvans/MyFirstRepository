...
variables and create a table of the number of cases for each combination. An
introductory statistics textbook contains a dataset summarizing student surveys
from several sections of an intro class. The two variables of interest for us are
`Gender` and `Year` which are the students gender and year in college.
  a) Download the dataset and correctly order the `Year` variable using the 
     following:
```{r}
    Survey <- read.csv('StudentSurvey.csv', na.strings=c('',' ')) 
```
  ...
  b) Using some combination of `dplyr` functions, produce a data set with
     ...
     *`dplyr::drop_na`.* 
```{r}
    count <- Survey %>% count(Sex,Year) %>% drop_na()
      count
```
  ...
  c) Using `tidyr` commands, produce a table of the number of responses in
     the following form:
```{r}
    table <- count %>% pivot_wider( names_from = Year, values_from = n)
      table
```
...
```{r}
    max.temp <- read.csv("https://raw.githubusercontent.com/dereksonderegger/444/master/data-raw/FlagMaxTemp.csv")
      max.temp2005 <- max.temp %>% filter(Year == "2005")
      max.pivot <- max.temp2005 %>% 
        pivot_longer(X1:X31, names_to = "Day", values_to = "Max Temperature")
      max.pivot <- max.pivot %>% 
        mutate(Day = str_replace_all(Day, pattern="X", replacement=""))
      max.pivot <- max.pivot %>% 
        mutate(Date = make_date(year=Year, month = Month, day = Day))
      ggplot(data=max.pivot, aes(x=Date, y= `Max Temperature`)) + geom_path()
```
...
\newpage
  b) Create a line graph that gives the monthly average maximum temperature
     ...
     *spans 3 years.*
```{r}
    max.monthly <- max.temp %>% 
      filter(Year == "2013" | Year == "2014" | Year == "2015")
    pivot.monthly <- max.monthly %>%
      pivot_longer(X1:X31, names_to = "Day", values_to = "Max Temperature")
    pivot.monthly <- pivot.monthly %>%
      mutate(Day = str_replace_all(Day, pattern="X", replacement=""))
    pivot.monthly <- pivot.monthly %>%
      mutate(Date = make_date(year=Year, month = Month, day = Day))
    monthly.group <- pivot.monthly %>% drop_na() %>% group_by(Year, Month) %>% summarize(`Average Temperature` = mean(`Max Temperature`))
    monthly.group <- monthly.group %>%
      mutate(Date = make_date(year=Year, month = Month))
    ggplot(data=monthly.group, aes(x=Date, y= `Average Temperature`)) + geom_path()
...
```{r}
    AB <- full_join(A, B, by=join_by(Name == First.Name))
      AB
```
...
```{r}
    B.gp <- add_row(B, First.Name = "Alice", Pet = "Guinea Pig")
      B.gp
```
...
```{r}
    AB.join <- full_join(A, B.gp, by=join_by(Name == First.Name))
      AB.join
```
